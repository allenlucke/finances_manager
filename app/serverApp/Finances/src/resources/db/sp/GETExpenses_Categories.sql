...
--	END IF;
	...
	END IF;
    IF (RTRIM(name)IS NOT NULL)THEN
        EXPSTR := ' AND expenses_categories.name = ''' || V_EXNAM || ''' ';
	END IF;
...
--        PAISTR := ' AND expenses_categories.paid = ''' || V_PAID_ || ''' ';
--	END IF;
    ...
    IF (RTRIM(recurring)IS NOT NULL AND RTRIM(recurring) != 2 )THEN
        RCGSTR := ' AND expenses_categories.recurring = ''' || V_RCURG || ''' ';
	END IF;
	...
	END IF;
    IF (RTRIM(amount_due)IS NOT NULL)THEN
        ADUSTR := ' AND expenses_categories.amount_due = ''' || V_AMDUE || ''' ';
	END IF;
	...
	END IF;
    IF (RTRIM(amount_paid)IS NOT NULL)THEN
        APASTR := ' AND expenses_categories.amount_paid = ''' || V_AMPAI || ''' ';
	END IF;
	...
	SQLSTR := SQLSTR || 'SELECT expenses_categories.id, expenses_categories.name, expenses_categories.due_by, ';
    SQLSTR := SQLSTR || 'expenses_categories.paid_on, expenses_categories.recurring, expenses_categories.amount_due, ';
    SQLSTR := SQLSTR || 'expenses_categories.amount_paid, expenses_categories.users_id,  ';
    SQLSTR := SQLSTR ||  'expenses_categories.period_id FROM expenses_categories ';
...
--    SQLSTR := SQLSTR || PAISTR ;
    ...
    SQLSTR := SQLSTR || 'AND expenses_categories.due_by >= :V_STDAT AND expenses_categories.due_by < :V_ENDAT ';
